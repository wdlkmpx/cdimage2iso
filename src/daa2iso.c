...
//#define DAA_DEBUG 1
#define DAA_USE_ZLIB 1

#ifdef DAA_USE_ZLIB
#include <zlib.h>
#else
#include "daa2iso_tinflate.h" // http://www.ibsensoftware.com
#define z_stream void
#endif

#if defined(DAA_USE_ZLIB) && !defined(HAVE_LOCAL_ZLIB)
#define DAA_USE_ZLIB_CRC 1 // the local zlib might not include crc32.c/h
#else
...
#if !defined(DAA_USE_ZLIB)
static int swapped_btype[3] = { 0, 1, 2 };
#endif
...
int daa2iso (struct file_info *infile, struct file_info *outfile)
{
    ...
    if (!strncmp(signature, "DAA", 16) || !strncmp(signature, "\xb8\xbd\xb6", 16)) {
        ...
        if (!strncmp(signature, "DAA VOL", 16) || !strncmp(signature, "GBI VOL", 16)) {
            ...
            if ((ans[0] == 'y') || (ans[0] == 'Y')) {
                daagbi = TYPE_DAA;
            } else if (!strcasecmp(ans, "daa")) {
                daagbi = TYPE_DAA;
            } else if (!strcasecmp(ans, "gbi")) {
                daagbi = TYPE_GBI;
            } else {
                exit_error(0);
            }

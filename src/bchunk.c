...
static char *cuefile = NULL;
static int verbose = 0;
static int psxtruncate = 0;
static int raw = 0;
static int swabaudio = 0;
static int towav = 0;
...
static void parse_args(int argc, char *argv[])
{
	...
	while ((s = getopt(argc, argv, "swvp?hr")) != -1) {
		switch (s) {
			...
			case 'w': towav = 1; break;
			case 'p': psxtruncate = 1; break;
			case 's': swabaudio = 1;   break;
			case '?':
			case 'h':
	...
	while (optind < argc) {
		switch (argc - optind) {
			case 3: binfile = strdup(argv[optind]);  break;
			case 2: cuefile = strdup(argv[optind]);  break;
			case 1: basefile = strdup(argv[optind]); break;
			default:
				fprintf(stderr, "%s", USAGE);
				exit(1);
		}
...
static void gettrackmode(struct track_t *track, char *modes)
{
...
static char *progressbar(float f, int l)
{
	static char s[80];
	int i, n;
...
static int writetrack(FILE *bf, struct track_t *track, char *bname)
{
	char *fname;
	FILE *f;
	char buf[SECTLEN+10];
...
static char * w_str_change_ext (const char * fname, const char * newext)
{
    // fname[.ext] .newext = fname.newext
    // returns a string that must be freed
    char *tmp, *dot, *new_name;
    unsigned int len;
    if (!fname || !newext) {
        return NULL;
    }
    ...
    dot = strrchr (tmp, '.');
    if (dot) *dot = 0;
    len = strlen(tmp) + strlen(newext) + 3;
    new_name = (char*) malloc (len);
    snprintf (new_name, len, "%s%s", tmp, newext);
    free (tmp);
    return new_name;
}
...
int bchunk (struct file_info *infile, struct file_info *outfile)
{
    ...
    for (p = program_options.flags; *p; p++)
    {
        switch (*p) {
            ...
            case 's': swabaudio = 1;   break;
            default:
                printf ("*** Bchunk warning: '%c' unknown flag\n", *p);
        }
    }
    ...
    cuefile = w_str_change_ext (binfile, ".cue");
    cuef = fopen (cuefile, "r");
#ifndef _WIN32
    if (!cuef) {
        free (cuefile);
        cuefile = w_str_change_ext (binfile, ".CUE");
    ...
    basefile = strdup (outfile->path);
    clear_fileinfo (outfile, 1); // don't need it
    p = strrchr (basefile, '.');

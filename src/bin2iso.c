...
static unsigned char buf[SIZERAW + 100];
...
typedef struct track
{
   ...
   unsigned long idx0;
   unsigned long idx1;
   char num[3];   // always treated as char*
   char name[80]; // always treated as char*
...
static void dotrack(short mode, long preidx, long startidx, long endidx, unsigned long offset) 
{
   ...
   switch(mode)
   {
      ...
      case MODE1_2352: printf("Mode1/2048"); break;
      case MODE2_2336: printf("Mode2/2352"); break;
      case MODE2_2352:
         if(mode2to1 != 1) 
            printf("Mode2/2352");
         else 
      ...
      for(i=nTracks-1; i>=0; i--) {
         ...
         if( (doOneTrack == 0) && (doInPlace == 1) ) {
            if( (i != 0) || ( (i == 0) && ((trackA.mode == AUDIO)||(trackA.mode == MODE2_2336)) ) ) {
               ...
               printf("Truncating to %lld bytes\n", writepos);
